...
void Pet::eat(int amt) {
  m_health++; // Increase the pet's health by the amount
}

void Pet::play() {
  
  m_health--; // Decrease pet's health by 1 for the energy consumed
}

string Pet::name() const {
  return m_name; //  Return the pets' names.
}

int Pet::health() const {
  return m_health; // Return the pet's current health level.
}

bool Pet::alive() const {
  return m_health; // Return whether pet is alive.A pet is alive if its health is greater than zero.)
}
